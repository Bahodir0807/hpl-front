...
const catalogs = {
  ...
  panelSizes: [
    ...
    { id: 'size-1830', widthMm: 1830, heightMm: 3050, displayName: '1830 × 3050' },
    { id: 'size-sq', widthMm: 1000, heightMm: 1000, displayName: '1000 × 1000' },
  ],
  qualityClasses: [
...
describe('CalculationRequestForm', () => {
  it('lets a manager add a second calculation and multiple HPL rows', async () => {
    ...
    await user.click(screen.getByRole('button', { name: ADD_CALCULATION_LABEL }));
    expect(screen.getByText('Расчёт №2')).toBeInTheDocument();
    expect(screen.getAllByLabelText(/Тип HPL, строка/).length).toBeGreaterThan(2);
  });
  ...
  it('duplicates a row without keeping the persisted id', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Дублировать строку 1' }));
    expect(screen.getAllByLabelText(/Тип HPL, строка/)).toHaveLength(2);
  ...
  it('does not show a manufacturer field in the manager form', () => {
    ...
    expect(screen.queryByLabelText(/Производитель, строка/)).not.toBeInTheDocument();
  });
  ...
  it('keeps the selected global quality class when panel type changes', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Линейка, строка 1' })).toHaveTextContent(
      'Медиум',
    );
  ...
  it('does not invent quality classes when the global backend catalog is empty', async () => {
    ...
    expect(screen.queryByRole('button', { name: /^Эконом/ })).not.toBeInTheDocument();
  });

  it('shows customTypeDescription for catalog type other', async () => {
    const user = userEvent.setup();
    render(<Harness />);
    expect(screen.queryByLabelText(CUSTOM_TYPE_DESCRIPTION_LABEL)).not.toBeInTheDocument();
    await user.selectOptions(
      screen.getByLabelText('Тип HPL, строка 1'),
      'type-other',
    );
    expect(screen.getByLabelText(CUSTOM_TYPE_DESCRIPTION_LABEL)).toBeInTheDocument();
  });
  ...
  it('shows row validation and the submit-to-head action without convert/finalize', async () => {
    ...
    expect(screen.getByRole('button', { name: SUBMIT_TO_HEAD_LABEL })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Создать черновик КП' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Сформировать КП' })).not.toBeInTheDocument();
    expect(screen.queryByLabelText(/Утверждённая цена/)).not.toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: SUBMIT_TO_HEAD_LABEL }));
  ...
  it('keeps the standard size select visible and maps Decor to a PanelColor id', async () => {
    ...
    expect(screen.queryByLabelText('Ширина мм, строка 1')).not.toBeInTheDocument();
    expect(screen.queryByLabelText('Высота мм, строка 1')).not.toBeInTheDocument();
    expect(
  ...
  it('does not render an editable sheetsCount field', () => {
    ...
    expect(screen.getByLabelText('Количество, строка 1').tagName).not.toBe('INPUT');
  ...
  it('recalculates the preview when area or size changes and ceils fractions', async () => {
    ...
    await user.selectOptions(screen.getByLabelText('Размер, строка 1'), 'size-1');
    expect(screen.getByLabelText('Количество, строка 1')).toHaveTextContent(
      '336 шт.',
    );
